...
// Main InventoryTab Component — NOW ROUTER-COMPATIBLE
const InventoryTab = () => { // 👈 Removed all props
  const { t } = useLanguage();
            ...
            <div className="bg-red-50 border border-red-200 rounded p-3 mb-4">
              <div className="flex">
                ...
                <div className="ml-2">
                  ...
                  <button onClick={() => loadVehicles()} className="mt-1 text-xs text-red-700 font-medium">
                ...
                <table className="min-w-full divide-y divide-gray-200 text-xs">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {currentVehicles.length === 0 ? (
                      <tr><td colSpan="6" className="px-3 py-4 text-center text-xs text-gray-500">
                      ...
                      currentVehicles.map((vehicle) => (
                        <tr key={vehicle.id} className="hover:bg-gray-50">
                          ...
                          <td className="px-3 py-2 whitespace-nowrap text-xs text-gray-900">{vehicle.stockNumber}</td>
                          <td className="px-3 py-2 whitespace-nowrap text-xs text-gray-900">{vehicle.mileage?.toLocaleString()} km</td>
                          <td className="px-3 py-2 whitespace-nowrap">
                            {vehicle.status === 'sold' ? (
                              <div className="flex items-center text-green-600 font-medium">
                                <FiDollarSign className="h-3 w-3 mr-0.5" /> €{vehicle.finalSalePrice?.toLocaleString()}
                              </div>
                            ) : vehicle.status === 'active' ? (
                              <div>
                                <div className="flex items-center text-[#3b396d] font-medium">
                                  <FiDollarSign className="h-3 w-3 mr-0.5" /> €{vehicle.currentBid?.toLocaleString()}
                                </div>
                              ...
                              <div className="flex items-center text-gray-600 font-medium">
                                <FiDollarSign className="h-3 w-3 mr-0.5" /> €{vehicle.askingPrice?.toLocaleString()}
                              </div>
                            ) : (
                          ...
                          <td className="px-3 py-2 whitespace-nowrap text-xs font-medium">
                            <div className="flex items-center space-x-1">
                              ...
                              <button
                                ...
                                className={`${vehicle.featured ? 'text-yellow-500' : 'text-gray-400'} p-0.5 rounded hover:bg-gray-100`}
            ...
            <div className="flex items-center justify-between bg-white px-3 py-2 border-t border-gray-200 rounded-lg text-xs mt-4">
              <div className="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                ...
                <nav className="isolate inline-flex -space-x-px rounded shadow-sm">
                  ...
                  {totalPages > 5 && <span className="px-2 py-1.5 text-xs text-gray-700">...</span>}
                  {totalPages > 5 && (
                    <button
